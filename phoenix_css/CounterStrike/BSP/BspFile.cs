...
namespace Phoenix.CounterStrike.BSP
{
    internal class BspFile
    {
        ...
        {
			var gameDir = EngineClient.GameDirectory;
			var mapDir = gameDir + "\\" + filePath;
			if (!File.Exists(mapDir))
				mapDir = gameDir + "\\download\\" + filePath;
			while (!File.Exists(mapDir))
				Thread.Sleep(10);
			while (!IsFileLocked(new FileInfo(mapDir)))
				Thread.Sleep(10);
            using (FileStream stream = File.OpenRead(mapDir))
            {
                Load(stream);
            }
            Console.WriteSuccess("\nLoaded " + mapDir.Split('\\').Last());
