...
namespace Phoenix.ConsoleSystem
{
    internal class Console
    {
        #region VARIABLES
        private static string CommandLine { get; } = ">" + System.Environment.UserName + ": ";
        public static string Watermark { get; } = $"PhoenixCSS v{Assembly.GetExecutingAssembly().GetName().Version.ToString()} ({Assembly.GetExecutingAssembly().GetLinkerTime()})";
        public static string Credits { get; } = "by Requi @ www.requi-dev.de";
        #endregion
        ...
        public static void WriteWatermark()
        {
            ...
            WriteLine(string.Format("{0," + ((WindowWidth / 2) + (Credits.Length / 2)) + "}", Credits));
            Write("\n\n\n");
        }
        #endregion
    }
